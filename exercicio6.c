...
int main()
{
    ...
    for (i = 0; i < 3; i++) // Loop para percorrer as linhas da matriz
    {
        for (j = 0; j < 3; j++) // Loop para percorrer as colunas da matriz
        {
            ...
            if (i == j) // Verifica se a posição é da diagonal principal
            {
                soma += matriz[i][j]; // Acumula a soma dos elementos da diagonal principal
            }
        }
    }

    printf("Soma da diagonal principal: %d\n", soma); // Exibe a soma dos elementos da diagonal principal
